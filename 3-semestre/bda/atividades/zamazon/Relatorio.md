...
-- view com quantidade de itens vendidos por categoria e o preco medio dos itens 

```sql
drop view if exists quantVendas;
create view quantVendas as 
select c.descricao categoria, sum(pp.quantidade) quantVendidos, avg(pp.preco) media
from pedido_produto pp inner join pedido p on pp.pedido_id=p.id 
